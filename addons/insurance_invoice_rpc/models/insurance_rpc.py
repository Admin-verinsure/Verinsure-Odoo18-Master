...
class InsuranceDetails(models.Model):
    ...
    def rpc_create_insurance_invoice_and_email(self, payload: dict):
        ...
        # Partner by email (avoid writing email again due to your duplicate-email validator)
        ...
        # IMPORTANT: Cybro UI screenshot shows "Source Document" = INS/006.
        # That's account.move.invoice_origin. Set it to insurance.name so the built-in invoice tab shows it.
        move = self.env["account.move"].sudo().with_company(company).create({
            ...
            "invoice_user_id": self.env.user.id,   # helps 'My Invoices' filters
            "invoice_origin": insurance.name,      # <-- makes it appear as Source Document under insurance in Cybro UI
            "ref": insurance.name,                 # optional: also tag reference
            "insurance_details_id": insurance.id,  # explicit link for reporting
