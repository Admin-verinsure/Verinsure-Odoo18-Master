...
class AutoReconciliationConfig(models.Model):
    ...
    def action_open_wizard(self):
        """
        CRITICAL FIX 3: Build two separate datasets for the wizard:
          - match_summary: JSON list for display (human-readable)
          - match_pairs_json: ID pairs only — confirm() applies exactly
            these records, re-validated at confirm time.
        """
        self.ensure_one()
        engine = self.env['auto.reconciliation.engine']
