...
# Monetary precision used for float-safe comparisons (2 decimal places = cents)
...
def _amounts_match(a, b):
    """
    CRITICAL FIX 1: Safe monetary comparison.
    float_compare returns 0 when |a - b| < 10^(-precision).
    Replaces bare == / ORM '=' domain comparisons on float monetary fields.
    """
...
class AutoReconciliationEngine(models.Model):
    ...
    def _reconcile_bank_statements(self, company, preview_mode=False):
        ...
        for stmt_line in stmt_lines:
            ...
            # Currency-aware amount matching
            if is_foreign:
                sign = 1 if stmt_line.amount >= 0 else -1
                signed_amount = sign * match_amount
                # CRITICAL FIX 1: Use range instead of exact float equality on amount_currency
                domain += [
                    ('currency_id', '=', stmt_currency.id),
                    ('amount_currency', '>=', signed_amount - AMOUNT_TOLERANCE),
                    ('amount_currency', '<=', signed_amount + AMOUNT_TOLERANCE),
                ]
            else:
                # CRITICAL FIX 1: Use range instead of exact float equality on debit/credit
    ...
    def _apply_bank_reconciliation_community(self, stmt_line, move_line):
        ...
        CRITICAL FIX 2: The original fallback used `l.id != move_line.id` to exclude
        the external move line from the statement's own lines. But move_line lives on a
        different account.move entirely — its id will never appear in stmt_line.move_id.line_ids,
        so the condition was always True and the fallback grabbed any unreconciled line
        (including the bank account line itself), risking corrupt journal entries.

        Correct approach:
          1. Primary: look for receivable/payable lines on the statement move (standard path).
          2. Fallback: look specifically for the journal's suspense/transit account line,
             identified by the journal's configured suspense_account_id. This is the account
             Odoo uses to temporarily park unreconciled bank lines in Odoo 18.
        """
        try:
            # Primary path: statement move already has a receivable/payable line
            ...
            # CRITICAL FIX 2: Correct suspense fallback — target the journal's suspense
            # account explicitly, not just "any unreconciled line that isn't move_line".
            ...
            # Last resort: any outstanding current-asset or current-liability line
            # on the statement move (catches non-standard suspense account setups)
            ...
            if fallback:
                ...
                _logger.warning(
                    "Bank recon: no suitable line found on stmt_line %s move to reconcile against move_line %s",
    ...
    def _reconcile_customer_payments(self, company, preview_mode=False):
        ...
        for payment in Payment.search([
            ...
            # ('amount_residual', '=', payment.amount) silently fails when IEEE 754
            # rounding causes tiny differences between the two stored floats.
            ...
            # Secondary guard: confirm with float_compare
    ...
    def _reconcile_vendor_payments(self, company, preview_mode=False):
        ...
        for payment in Payment.search([
            ...
            # CRITICAL FIX 1: Same float-safe tolerance range as customer payments above
            amt = float_round(payment.amount, precision_digits=MONETARY_PRECISION)
            bills = Move.search([
    ...
    def _reconcile_intercompany(self, company, preview_mode=False):
        ...
        for line in ic_lines:
            ...
            # Currency-aware IC matching with CRITICAL FIX 1 tolerance ranges
