...
class SelectFieldConfigWizard(models.TransientModel):
    ...
    option_source = fields.Selection([
        ('manual', 'Manual'),
        ('model', 'From Model'),
    ], string="Options Source", default='manual')
    option_model_id = fields.Many2one('ir.model', string="Source Model")
    option_domain = fields.Char(string="Source Domain", help="e.g. [('active','=',True)]")
    ...
    def default_get(self, fields_list):
        ...
        if field_id:
            ...
            defaults.update({
                ...
                'option_source': field.option_source or 'manual',
                'option_model_id': field.option_model_id.id if field.option_model_id else False,
                'option_domain': field.option_domain,
                'option_label_field': field.option_label_field or 'name',
                'option_value_field': field.option_value_field or 'id',
    ...
    def action_save_select_config(self):
        self.field_id.write({
            ...
            'option_model_id': self.option_model_id.id if self.option_model_id else False,
            'option_domain': self.option_domain,
            'option_label_field': self.option_label_field,
            'option_value_field': self.option_value_field,
