...
class SmartFormPublic(http.Controller):
    ...
    def smart_form_page(self, token, **kw):
        ...
        return request.render("smart_form_builder.smart_form_page", {"form": form, "rules_json": rules_json})
    ...
    def smart_form_options(self, field_id, token=None, **kw):
        ...
        return request.make_response(json.dumps({"success": True, "options": field.get_options()}), [("Content-Type", "application/json")])
    ...
    def smart_form_branching(self, token, **kw):
        ...
        def _vals(val):
            if isinstance(val, dict):
                vs = []
                if "value" in val: vs.append(str(val.get("value") or "").strip())
                if "label" in val: vs.append(str(val.get("label") or "").strip())
    ...
    def smart_form_submit(self, **post):
        ...
        submission.sudo().write({"data_json": json.dumps(data, ensure_ascii=False)})
