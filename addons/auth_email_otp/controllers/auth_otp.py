...
def _get_challenge(env, challenge_id: int, user_id: int):
    """
    Fetch the auth.otp.challenge record with minimal trusted query.
...
class AuthOtpController(http.Controller):
    ...
    def web_login_post(self, redirect=None, **post):
        ...
        2. If authentication fails → fall through to native error handling.
        3. If user has 2FA disabled → allow native flow to complete (do nothing).
        4. If user has 2FA enabled:
           a. Clear the just-created session (prevent session fixation).
           b. Store pending state in a NEW session.
           c. Generate OTP challenge and send email.
           d. Redirect to /auth/otp/verify.

        We deliberately call `request.session.authenticate()` ourselves so we
        don't need to patch the Home controller. By returning our own response
        when 2FA is needed, we effectively take over the flow for those users.
        """
        db = request.db
        ...
        if not user.email_otp_enabled:
            # 2FA not required — native flow already completed auth, just redirect
            redirect_url = _safe_redirect(redirect or request.params.get('redirect') or '/odoo')
            return request.redirect(redirect_url)

        # --- Step 3: 2FA required ---
        # Check user has an email address
        if not user.email:
            _logger.error(
                'auth.otp: User %s (id=%d) has 2FA enabled but no email address — blocking login',
                user.login, uid,
            )
        ...
        # Wipe the authenticated session to prevent session fixation.
        # The user is NOT logged in yet — we start a fresh anonymous session.
        request.session.logout(keep_db=True)

        # Create OTP challenge
        ip_address = request.httprequest.environ.get('HTTP_X_FORWARDED_FOR',
                     request.httprequest.environ.get('REMOTE_ADDR', ''))
        # Take first IP if comma-separated (reverse proxy)
        ...
        try:
            ...
            return self._render_login_error(
                _('Could not send verification code. Please try again or contact your administrator.')
            )

        # Store pending auth state in session
        request.session[_SESSION_UID] = uid
        request.session[_SESSION_CHALLENGE] = challenge.id
        request.session[_SESSION_REDIRECT] = _safe_redirect(redirect or request.params.get('redirect') or '/odoo')
    ...
    def _fallback_to_native_login(self, redirect=None, **post):
        """
        Render the standard login page with Odoo's own error handling.
        We replicate what Home.web_login() would do on failure.
        """
        # Re-use Odoo's own login page render with error
        values = {
    ...
    def _render_login_error(self, message: str):
        """Render the login page with a custom error message."""
        return request.render('web.login', {'error': message})

    @staticmethod
    def _send_otp_email(env, user, plain_otp: str):
        """Send the OTP via Odoo mail template."""
        template = env.ref('auth_email_otp.email_template_otp_code', raise_if_not_found=True)
    ...
    def otp_verify_get(self, **kwargs):
        ...
        if not uid or not challenge_id or db != request.db:
            _logger.warning('auth.otp: GET /auth/otp/verify accessed without valid session state.')
        ...
        if not challenge:
            _logger.warning(
                'auth.otp: No valid challenge found for uid=%d challenge_id=%d — redirecting to login.',
                uid, challenge_id,
            )
            _clear_otp_session()
            return request.redirect('/web/login')
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # OTP Verification — POST (submit code)
    # ──────────────────────────────────────────────────────────────────────────
    ...
    def otp_verify_post(self, otp_code='', **kwargs):
        ...
        - Session state validated (uid, challenge_id, db)
        - Challenge existence and state validated
        - OTP stripped and length-checked (rejects obvious junk)
        - Delegates hash comparison to model (constant-time digest)
        - On success: finalise session, prevent fixation, redirect
        - On failure: generic error, never reveal details
        - On lockout: clear session, redirect to login
        """
        uid = request.session.get(_SESSION_UID)
        challenge_id = request.session.get(_SESSION_CHALLENGE)
        ...
        if not is_valid:
            ...
            if challenge.state == 'cancelled':
                # Lockout reached or challenge invalidated
                _clear_otp_session()
                _logger.warning(
                    'auth.otp: Challenge %d for uid=%d cancelled (lockout or expiry) — forcing re-login.',
                    challenge_id, uid,
                )
                return request.render('web.login', {'error': _LOCKOUT_ERROR})
    ...
    def otp_resend(self, **kwargs):
        ...
        - Creates a NEW challenge (cancels the old one)
        ...
        # create_challenge() cancels old challenge automatically
        ...
        try:
            ...
            _logger.exception('auth.otp: Failed to resend OTP to user %s: %s', user.login, str(e))
            new_challenge.sudo().write({'state': 'cancelled'})
            _clear_otp_session()
            return request.redirect('/web/login')

        # Update session to point to new challenge
    ...
    def otp_cancel(self, **kwargs):
        """Allow the user to abandon OTP flow and return to login."""
        challenge_id = request.session.get(_SESSION_CHALLENGE)
        uid = request.session.get(_SESSION_UID)
