 ...
 *
 * Targets: form#Ticket_form (action="/website/form/")
 * inside section.s_website_form on /helpdesk_ticket page.
 *
 * Injects:
 *   1. Program Type  <select name="hd_program_type_id">
 *   2. Club Name     <select name="hd_club_id"> + live search input
 */
...
function makeFormRow(labelText, ...inputEls) {
    ...
    wrap.className = "mb-3 s_website_form_field s_website_form_custom s_website_form_required col-lg-12";

    const lbl = document.createElement("label");
    lbl.className = "col-form-label s_website_form_label fw-bold";
    lbl.innerHTML = `${labelText} <span class="s_website_form_mark text-danger">*</span>`;
...
async function loadClubs(programTypeId, clubSel, searchInp) {
    ...
    try {
        ...
        console.error("[helpdesk_club_picker] club fetch error:", e);
...
async function loadProgramTypes(typeSel) {
    ...
    try {
        ...
        if (!types || !types.length) {
            typeSel.replaceChildren(makeOption("", "-- No program types configured --"));
            console.warn("[helpdesk_club_picker] No program types in DB. Go to Helpdesk > Program Types to add some.");
            return;
        }
        ...
        console.error("[helpdesk_club_picker] program type fetch error:", e);
        typeSel.replaceChildren(makeOption("", "-- Error loading program types --"));
    }
}

// ─── Find the helpdesk form ───────────────────────────────────────────────────
function findHelpdeskForm() {
    // Primary: exact id from console inspection
    const byId = document.getElementById("Ticket_form");
    if (byId) return byId;

    // Fallback 1: action points to /website/form/ inside s_website_form section
    for (const sec of document.querySelectorAll("section.s_website_form")) {
        const f = sec.querySelector("form");
        if (f) return f;
    }

    // Fallback 2: any form whose action contains /website/form
    for (const f of document.querySelectorAll("form")) {
        if ((f.action || "").includes("/website/form")) return f;
    }

    return null;
}

// ─── Find insertion point (before submit button) ──────────────────────────────
function findInsertBefore(form) {
    // Look for .s_website_form_submit div first
    const submitDiv = form.querySelector(".s_website_form_submit");
    if (submitDiv) return submitDiv;

    // Walk up from the submit button to its direct form child
...
async function injectFields() {
    ...
    if (document.getElementById("hd_program_type_id")) return;

    const form = findHelpdeskForm();
    if (!form) {
        console.warn("[helpdesk_club_picker] Form#Ticket_form not found on this page. Skipping.");
        return;
    }

    console.log("[helpdesk_club_picker] Found form:", form.id || form.action);

    const insertBefore = findInsertBefore(form);

    // ── Build Program Type select ─────────────────────────────────────────
    const typeSel = makeSelect("hd_program_type_id", "hd_program_type_id");
    const typeRow = makeFormRow("Program Type", typeSel);

    // ── Build Club Name: search input + select ────────────────────────────
