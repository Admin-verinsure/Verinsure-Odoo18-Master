...
_module_ns = "odoo.addons.account_statement_import_online_akahu"
_interface_class = _module_ns + ".models.akahu_interface" + ".AkahuInterface"
...
FOUR_TRANSACTIONS = [
    # First transaction will be after date_until.
    {
        ...
        "relationships": {
            "account": {
                "links": {"related": "https://api.myakahu.com/accounts/"},
                "data": {
                    "type": "account",
...
class TestAccountStatementImportOnlineAkahu(common.TransactionCase):
    post_install = True
    ...
    def test_akahu_execution_date(self):
        with (
            self.mock_login(),
            self.mock_set_access_account(),
            self.mock_get_transactions(),
            ...
            self.provider.akahu_date_field = "execution_date"
    ...
    def test_akahu_value_date(self):
        with (
            self.mock_login(),
            self.mock_set_access_account(),
            self.mock_get_transactions(),
    ...
    def test_akahu_scheduled(self):
        with (
            ...
            self.provider.akahu_last_identifier = False
    ...
    def test_akahu_scheduled_from_identifier(self):
        with (
            self.mock_login(),
            self.mock_set_access_account(),
            self.mock_get_transactions(),
            ...
            self.provider.akahu_last_identifier = "9ac50483-16dc-4a82-aa60-df56077405cd"
            date_since = datetime(2019, 11, 3)
            date_until = datetime(2019, 11, 18)
            self.provider.with_context(scheduled=True)._pull(date_since, date_until)
