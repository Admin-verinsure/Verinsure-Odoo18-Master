...
from .test_account_statement_import_online_akahu import FOUR_TRANSACTIONS


class TestAkahuInterface(common.TransactionCase):
    post_install = True

    @patch("requests.post")
    def test_login(self, requests_post):
        """Check Akahu API login."""
    ...
    def test_set_access_account(self, requests_get):
        """Test getting account data for Akahu access."""
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.text = json.dumps(
            {
        ...
        self.assertIn("akahu_account", access_data)
        self.assertEqual(
            access_data["akahu_account"], "2ad3df83-be01-47cf-a6be-cf0de5cb4c99"
        )

    @patch("requests.get")
    def test_get_transactions(self, requests_get):
        """Test getting transactions from Akahu."""
        mock_response = MagicMock()
        mock_response.status_code = 200
